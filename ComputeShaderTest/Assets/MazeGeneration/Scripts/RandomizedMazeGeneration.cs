...
namespace MazeGeneration.Scripts
{
    public class RandomizedMazeGeneration : MonoBehaviour
    {
        ...
        private readonly Queue<Cell> pathQueue = new Queue<Cell>();
        ...
        private void MazeStep()
        {
            ...
            if (nextCell != null)
            {
                ...
                //Add to path for later backtracking
                if (isBreadthFirst)
                    pathQueue.Enqueue(currentCell);
                else
                    path.Push(currentCell);
            ...
            {
                //Backtrack using queue/stack until we either
                //find a neighbor, or empty the queue/stack
                if (isBreadthFirst)
                {
                    if (pathQueue.Count == 0) return;
                    currentCell = pathQueue.Dequeue();
                }
                else
                {
                    if (path.Count == 0) return;
